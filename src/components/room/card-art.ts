...
function paintFace(cv: Canvas, card: Card) {
  ...
  if (card.rank === 1) {
    ...
    // The ace of spades keeps its traditional flourish: a gold ring with four studs.
    if (card.suit === 'S') {
      const cx = CARD_W / 2
      const cy = CARD_H / 2
      for (let y = 0; y < CARD_H; y++) {
        for (let x = 0; x < CARD_W; x++) {
          if (Math.abs(Math.hypot(x + 0.5 - cx, y + 0.5 - cy) - 12.5) < 0.55) cv.set(x, y, COURT_PALETTE.y)
        }
      }
      for (const [x, y] of [[22, 19], [22, 43], [10, 31], [34, 31]]) {
        for (const [dx, dy] of [[0, 0], [1, 0], [-1, 0], [0, 1], [0, -1]]) cv.set(x + dx, y + dy, ink)
      }
    }
    return
  }
