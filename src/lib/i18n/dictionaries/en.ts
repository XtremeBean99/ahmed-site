/**
 * English dictionary, the canonical source of truth for all translatable UI
 * copy. Its shape defines the `Dictionary` type. Adding a key here requires
 * all consumers to handle it.
 */

export const en = {
  nav: {
    homeLink: 'Home',
    projects: 'Projects',
    games: 'Games',
    tutoring: 'Tutoring',
    cta: 'Get in touch',
    home: 'Ahmed Hussain, home',
    openMenu: 'Open menu',
    closeMenu: 'Close menu',
    primary: 'Primary navigation',
    skip: 'Skip to main content',
  },

  footer: {
    blurb:
      'All content on this website is copyrighted and may not be reproduced, scraped, indexed for AI training, or incorporated into generative AI systems without prior written permission.',
    projects: 'Projects',
    tutoring: 'Tutoring',
    privacy: 'Privacy Policy',
    terms: 'Terms of Use',
    linkedin: 'LinkedIn',
    email: 'Email',
    nav: 'Footer navigation',
    rights: 'Ahmed Hussain. All rights reserved.',
    location: 'Canberra, Australia\u00a0\u00b7 ahmedyhussain.com',
  },

  hero: {
    eyebrow: 'Canberra, Australia',
    descriptor1: 'BComp\u2009/\u2009LLB(Hons) student at the Australian National University.',
    descriptor2: 'Working toward a future in tech law',
    ctaPrimary: 'Connect on LinkedIn',
    ctaSecondary: 'Get in touch',
    ctaRoom: 'My room',
    intro: 'Introduction',
    scroll: 'Scroll',
  },

  about: {
    eyebrow: 'About',
    headingLine1: 'Law. Computing.',
    headingLine2: 'Technology.',
    p1: 'I am a double-degree student at the Australian National University pursuing a Bachelor of Computing alongside a Bachelor of Laws (Honours). My academic interest sits in the legal and ethical dimensions of artificial intelligence.',
    p2: 'I am interested in how legal systems adapt to fast technological change. I am actively working on technical and professional skills within this industry. Cybersecurity, privacy, and digital governance are areas I follow closely.',
    p3: 'Outside of study I tutor senior secondary students in mathematics, physics, English, and legal studies. They are subjects that reward a patient, structured approach, which is the way I like to teach.',
    imageAlt: 'Legal documents and a desk, representing legal study and practice',
  },

  interests: {
    eyebrow: 'Professional Interests',
    heading: 'Areas of focus.',
    cards: [
      { title: 'Legal Technology', desc: 'How software is transforming legal practice, access to justice, and judicial processes.' },
      { title: 'Software Engineering', desc: 'Building systems that are reliable and easy to reason about, with care taken over correctness and clarity.' },
      { title: 'Cybersecurity', desc: 'Technical security, vulnerability research, and the intersection of security with law.' },
      { title: 'Artificial Intelligence', desc: 'Machine learning systems, their limitations, and the emerging legal frameworks around them.' },
      { title: 'Privacy & Data', desc: 'Data protection law, surveillance, and the technical architecture of privacy.' },
      { title: 'Digital Governance', desc: 'Regulatory approaches to platform power, algorithmic accountability, and digital markets.' },
      { title: 'Technology Policy', desc: 'How governments legislate technology, and whether the rules we already have are fit for it.' },
      { title: 'Research', desc: 'Empirical and doctrinal legal research at the intersection of law and technical systems.' },
      { title: 'Emerging Technologies', desc: 'Blockchain, biometrics, autonomous systems, and their legal implications.' },
    ],
  },

  education: {
    eyebrow: 'Education',
    heading: 'Academic background.',
    university: 'Australian National University',
    location: 'Canberra, ACT, Australia',
    status: 'Expected 2031',
    degree1: 'Bachelor of Computing',
    degree1desc: 'Software engineering, algorithms, systems, and security',
    degree2: 'Bachelor of Laws',
    honours: '(Honours)',
    degree2desc: 'Common law, statutory interpretation, and emerging legal frameworks',
  },

  skills: {
    eyebrow: 'Skills',
    heading: 'Technical & legal capability.',
    programming: 'Programming',
    web: 'Web & Frameworks',
    security: 'Security & Infrastructure',
    legal: 'Legal',
    legalItems: [
      'Contract Law',
      'Administrative Law',
      'Constitutional Law',
      'Legal Research',
      'Statutory Interpretation',
    ],
  },

  contact: {
    eyebrow: 'Contact',
    heading: 'Let\u2019s talk.',
    intro: 'Whether you have a question about tutoring, want to talk about legal technology, or just want to connect, I would be glad to hear from you.',
    bullet1: 'Tutoring enquiries (Years 7\u201312)',
    bullet2: 'Professional introductions',
    bullet3: 'Research or academic collaboration',
    emailPromptBefore: 'Prefer email? Reach me directly at ',
    emailPromptAfter: '.',
  },

  contactForm: {
    name: 'Name',
    namePlaceholder: 'Your name',
    email: 'Email',
    emailPlaceholder: 'you@example.com',
    subject: 'Subject',
    subjectPlaceholder: 'What is this regarding?',
    message: 'Message',
    messagePlaceholder: 'Your message',
    send: 'Send message',
    sending: 'Sending\u2026',
    successTitle: 'Message sent.',
    successBody: 'Thank you for reaching out. I will be in touch shortly.',
    sendAnother: 'Send another message',
    errorRate: 'You have sent too many messages. Please try again in about an hour.',
    errorGeneric: 'Something went wrong. Please try again.',
    errorNetwork: 'Network error. Please check your connection and try again.',
    tutoringSubject: 'Tutoring enquiry',
    validation: {
      nameRequired: 'Name is required',
      nameMax: 'Name must be under 100 characters',
      emailInvalid: 'A valid email address is required',
      emailMax: 'Email address is too long',
      subjectRequired: 'Subject is required',
      subjectMax: 'Subject must be under 200 characters',
      messageMin: 'Message must be at least 10 characters',
      messageMax: 'Message must be under 5000 characters',
    },
  },

  projects: {
    eyebrow: 'Projects',
    heading: 'Selected work.',
    intro: 'A curated selection of my most recent professional & personal projects.',
    view: 'View project',
    viewExternal: 'Visit site',
    comingSoon: 'Coming soon',
    cards: {
      ninja: {
        label: 'Game',
        title: 'Super Space Monk Ninja Fighter Simulator IV',
        desc: 'A fast, movement-focused 2D platformer with a hand-drawn ink-and-void aesthetic. Wall-jump, slide, and turn your momentum into your main weapon across seven hand-crafted levels.',
      },
      code: {
        label: 'Open source',
        title: 'My Github Projects',
        desc: 'Public repositories pulled live from GitHub: the software side of my law-and-computing work.',
      },
      silicon: {
        label: 'Interactive',
        title: 'Silicon: from atom to architecture',
        desc: 'An interactive 3D model of a silicon atom, with an explainer on how its four valence electrons end up running every computer.',
      },
      aglc4: {
        label: 'Legal tool',
        title: 'AGLC4 citation generator',
        desc: 'Build footnote and bibliography citations in the Australian Guide to Legal Citation (4th ed) style for cases, legislation, articles, books, web pages, and Hansard, then copy them straight into your work.',
      },
      converter: {
        label: 'Computing tool',
        title: 'Base converter',
        desc: 'Convert live between decimal, binary, hex, octal and UTF-8 text, with a bitwise playground for AND, OR, XOR, NOT and shifts. Arbitrary-precision, all in the browser.',
      },
      dev: {
        label: 'Research',
        title: 'In development',
        desc: 'Empirical and doctrinal work on AI governance. I will publish it here as it develops.',
      },
    },
  },

  toolShell: {
    allProjects: 'All projects',
  },

  gameShell: {
    allGames: 'All games',
  },

  code: {
    eyebrow: 'Projects \u00b7 Code',
    heading: 'Code & open source',
    intro: 'Public repositories pulled live from GitHub: the software side of my law-and-computing work.',
    fallbackTitle: 'Unable to load repositories right now.',
    fallbackBody: 'The GitHub API may be temporarily unavailable or rate-limited. You can browse all repositories directly.',
    fallbackLink: 'github.com/XtremeBean99 \u2192',
    dataSource: 'Data source',
    dataSourceBody: 'Repository data is pulled live from the public GitHub REST API and refreshed hourly. Only public, non-fork, non-archived repositories are shown.',
  },

  silicon: {
    eyebrow: 'Projects \u00b7 Silicon',
    heading: 'Silicon: from atom to architecture',
    intro: 'An interactive 3D Bohr model of a silicon atom, atomic number 14, with 2 electrons in its first shell, 8 in the second, and 4 in the third. Drag to rotate, scroll to zoom. The four valence electrons are the reason every modern processor exists.',
    caption: 'Drag to rotate \u00b7 Scroll to zoom \u00b7 Reduced-motion users see a static render',
    s1heading: 'Why silicon',
    s1p1: 'Silicon sits in group 14 of the periodic table, right below carbon. It has four electrons in its outermost shell, exactly the number needed to form four covalent bonds and lock into a stable, repeating crystal lattice. Unlike a metal (which conducts electricity freely) or an insulator (which blocks it entirely), pure silicon is a <strong>semiconductor</strong>: it conducts electricity only when given a nudge.',
    s1p2: 'That in-between behaviour is what makes it controllable. Apply a voltage, and it switches from off to on. Remove the voltage, and it switches back. That switch, the binary state, is the physical basis for every logical 1 and 0 in digital computing.',
    s2heading: 'Doping: tuning the conductivity',
    s2p1: 'Pure silicon is not very conductive on its own. The trick is <strong>doping</strong>: introducing a tiny number of impurity atoms into the crystal.',
    s2p2: 'Add phosphorus (five valence electrons) and you get <strong>n-type</strong> silicon, which has spare electrons ready to move. Add boron (three valence electrons) and you get <strong>p-type</strong> silicon, which has \u201choles\u201d (gaps where an electron is missing) that behave like positive charge carriers.',
    s2p3: 'Place a piece of n-type silicon next to a piece of p-type silicon and you have a <strong>p-n junction</strong>, the building block of the diode. Current flows in one direction and is blocked in the other. Stack three layers, p-n-p or n-p-n, and you have the basis of the transistor.',
    s3heading: 'The transistor',
    s3p1: 'The modern transistor, the <strong>MOSFET</strong> (Metal-Oxide-Semiconductor Field-Effect Transistor), is a voltage-controlled switch. A small voltage applied to the \u201cgate\u201d terminal creates an electric field that opens or closes a conductive channel between the \u201csource\u201d and \u201cdrain.\u201d No mechanical parts, no moving pieces, just a field and a semiconductor channel.',
    s3p2: 'A single modern CPU contains billions of these switches, fabricated at a scale measured in nanometres. Each one flips on and off billions of times per second. Those on/off states, aggregated across billions of transistors, are the 1s and 0s of digital logic. Every line of code you write eventually resolves to voltages across MOSFET gates.',
    s4heading: 'From sand to CPU',
    s4intro: 'Silicon does not come out of the ground ready for a logic gate. The journey from raw material to integrated circuit is one of the most precise manufacturing processes ever devised.',
    s4b1: '<strong>Refinement.</strong> Quartz (silicon dioxide, SiO\u2082) is reduced with carbon in an arc furnace to produce metallurgical-grade silicon, then further purified to electronic-grade: 99.9999999% pure.',
    s4b2: '<strong>Crystal growth.</strong> The <strong>Czochralski process</strong> draws a single-crystal ingot from a melt: a flawless cylinder of silicon atoms in a perfect lattice, up to 300 mm across and a metre long.',
    s4b3: '<strong>Wafers.</strong> The ingot is sliced into wafers thinner than a human hair, polished to a mirror finish.',
    s4b4: '<strong>Photolithography.</strong> A light-sensitive resist is applied, exposed through a mask that carries the circuit pattern, and developed. Unprotected silicon is etched away. The process is repeated dozens of times, layer upon layer, to build up the transistors, interconnects, and isolation structures.',
    s4b5: '<strong>Packaging.</strong> The finished die is cut from the wafer, bonded to a substrate, and sealed in a package with electrical contacts, the black rectangle we recognise as a chip.',
    s5label: 'Why this matters here',
    s5body: 'Every question of AI governance, every cybersecurity regulation, every dispute over data sovereignty: they all ultimately run on this physical substrate. A silicon atom, doped and patterned, switches on and off a few billion times a second.',
  },

  games: {
    eyebrow: 'Games',
    heading: 'My Games',
    intro: 'A few small things built for fun and to keep the canvas and animation muscles warm. They run entirely in your browser and keep your best score on your device.',
    play: 'Play',
    cards: {
      ninja: {
        label: 'Platformer',
        title: 'Super Space Monk Ninja Fighter Simulator IV',
        desc: 'A fast, movement-focused 2D platformer with a hand-drawn ink-and-void aesthetic. Wall-jump, slide, and turn your momentum into your main weapon across seven hand-crafted levels.',
      },
      typing: {
        label: 'Live WPM',
        title: 'Typing speed test',
        desc: 'Type curated phrases on law, AI governance and cybersecurity while a live tracker measures your words per minute and accuracy.',
      },
      breakout: {
        label: 'Arcade',
        title: 'Breakout',
        desc: 'A monochrome take on the Atari classic: clear the wall, catch falling power-ups, and chase a personal best.',
      },

    },
  },

  typing: {
    eyebrow: 'Live WPM',
    heading: 'Typing speed test',
    intro: 'Type the phrase as accurately and quickly as you can. The tracker starts on your first keystroke and your best score stays on this device.',
    wpm: 'WPM',
    accuracy: 'Accuracy',
    bestWpm: 'Best WPM',
    focusArea: 'Focus typing area',
    typeHere: 'Type the phrase above',
    tapToType: 'Tap here to start typing',
    resultBefore: 'WPM at ',
    resultAccuracyIn: '% accuracy in ',
    resultSeconds: 's.',
    newBest: ' New personal best.',
    tryAnother: 'Try another',
    restart: 'Restart',
    startHint: 'Start typing to begin. Backspace to correct.',
  },

  breakout: {
    eyebrow: 'Arcade',
    heading: 'Breakout',
    intro: 'Move the paddle to keep the ball alive and clear every brick. Some bricks drop power-ups. Use mouse, touch, or the arrow keys, and tap or press space to launch.',
    time: 'Time',
    lives: 'Lives',
    best: 'Best',
    launch: 'Press space or tap to launch',
    start: 'Launch',
    paused: 'Paused',
    resume: 'Resume',
    pause: 'Pause',
    restart: 'Restart',
    gameOver: 'Game over.',
    clearedBefore: 'Cleared in ',
    clearedAfter: 's. Score ',
    playfield: 'Breakout game playfield',
    instructions: 'Mouse or touch to move. Arrow keys or A and D also work. Space launches and pauses, P pauses, R restarts. On a touchscreen, drag to move and use the buttons below to launch, pause, or restart. Clear the wall fast: the longer you take, the lower your score. E widens the paddle, M splits the ball, S slows it down, plus adds a life.',
    score: 'Score',
    topScores: 'Top scores',
  },

  aglc4: {
    eyebrow: 'Legal tool',
    heading: 'AGLC4 citation generator',
    intro: 'Build footnote and bibliography citations in the Australian Guide to Legal Citation (4th ed) style. Choose a source type, fill in the fields, and the citation updates as you type, ready to copy into your essay or memo.',
    sourceType: 'Source type',
    example: 'Example:',
    clearFields: 'Clear fields',
    footnote: 'Footnote',
    bibliography: 'Bibliography',
    fillPrompt: 'Fill in the fields to build a citation.',
    copy: 'Copy',
    copied: 'Copied',
    copyAria: 'Copy citation',
    disclaimer: 'A study aid based on AGLC4. It covers common cases, and you should always check the output against the guide before relying on it. Italics are shown as you would type them in a document.',
  },

  baseConverter: {
    eyebrow: 'Computing tool',
    heading: 'Base converter',
    intro: 'Convert between decimal, binary, hexadecimal, octal and UTF-8 text. Edit any field and the rest follow live; then experiment with AND, OR, XOR, NOT and shifts in the bitwise playground. Everything is arbitrary-precision and runs entirely in your browser.',
    basesTitle: 'Number bases & text',
    helper: 'Edit any field and the rest update live. Values are arbitrary-precision, so very large numbers work. Text maps each character to its UTF-8 bytes.',
    decimal: 'Decimal',
    binary: 'Binary',
    octal: 'Octal',
    hexadecimal: 'Hexadecimal',
    text: 'Text',
    base10: 'Base 10',
    base2: 'Base 2',
    base8: 'Base 8',
    base16: 'Base 16',
    utf8: 'UTF-8',
    typeText: 'Type some text\u2026',
    invalidBefore: 'Not a valid ',
    invalidAfter: ' value.',
    bitwiseTitle: 'Bitwise playground',
    aDecimal: 'A (decimal)',
    bDecimal: 'B (decimal)',
    operation: 'Operation',
    shiftBy: 'Shift by',
    result: 'Result',
    enterOperands: 'Enter valid operands to see the result.',
  },

  ninjaGame: {
    eyebrow: 'Platformer',
    heading: 'Super Space Monk Ninja Fighter Simulator IV',
    intro: 'A fast 2D platformer with a hand-drawn ink-and-void aesthetic. Run, jump, wall-jump, and slide through seven levels.',
    playHeading: 'Play',
    playBody: 'The game runs in your browser. Use the keyboard (WASD / arrows + Space) or a gamepad. Press Escape to pause, R to restart a level.',
    playButton: 'Launch game',
    controlsHeading: 'Controls',
    controls: [
      'Move: A/D or arrow keys',
      'Jump / double-jump: W, Space, or Up arrow',
      'Wall jump: Jump while against a wall',
      'Slide: S or Down while running',
      'Fireball: L (needs a charge)',
      'Quick restart: R',
      'Pause: Escape',
    ],
    featuresHeading: 'Features',
    features: [
      'Fluid movement with coyote time and jump buffering',
      'Combat through momentum: slide, stomp, and fireball',
      'Seven levels with ink-drawn terrain built at runtime from SVG strokes',
      'Ghost runner replays your previous attempt on each level',
      'Moving platforms, crumbling platforms, springs, and spikes',
      'Three enemy types: snails, flyers, and a boss',
      'Ambient parallax void background with drifting ink motes',
      'Live speedrun clock, HUD with health and token progress',
      'End-of-run screen with total time and fastest-run tracking',
    ],
    techHeading: 'Built with',
    techBody: 'Godot 4.7, GDScript, custom SVG-to-terrain pipeline, and hand-drawn sprite art. All terrain geometry is parsed from ink-stroke data at runtime.',
    creditsHeading: 'Credits',
    credits: [
      'Background music from YouTube (Ambient / Cinematic track)',
      'Player and enemy sprites by ansimuz, via opengameart.org',
      'All other art, code, and level design by Ahmed Hussain',
    ],
    leaderboardHeading: 'Leaderboards',
    leaderboardIntro:
      'Fastest verified runs, straight from the game. Finish a full run to post your time.',
    leaderboardHundred: '100% tokens',
    leaderboardAny: 'Any%',
    leaderboardRank: 'Rank',
    leaderboardName: 'Player',
    leaderboardTime: 'Time',
    leaderboardTokens: 'Tokens',
    leaderboardEmpty: 'No runs yet - be the first.',
    monkAlt: 'Hand-drawn monk artwork from the game title screen',
    bugsHeading: 'Report a bug',
    bugsIntro: 'Found something wrong? Send a description of what happened (and which level you were on) and I will fix it.',
    bugsName: 'Name (optional)',
    bugsEmail: 'Email (optional)',
    bugsDescription: 'What happened?',
    bugsDescriptionPlaceholder: 'Describe the bug and which level you were on\u2026',
    bugsSend: 'Send bug report',
    bugsSending: 'Sending\u2026',
    bugsSuccess: 'Thank you. Bug report sent - I will look into it.',
    bugsError: 'Something went wrong. Please try again or email me directly.',
  },

  ninja: {
    eyebrow: 'Projects \u00b7 Super Space Monk Ninja Fighter Simulator IV',
    heading: 'Super Space Monk Ninja Fighter Simulator IV',
    intro: 'A fast, movement-focused 2D platformer built in Godot 4.7 with a hand-drawn ink-and-void aesthetic. Seven hand-crafted levels are traced as ink strokes and built into terrain at runtime; you clear each one by flowing through it.',
    gameplayHeading: 'Gameplay',
    gameplayBody: 'Run, jump, double-jump, wall-jump, and slide through seven hand-crafted levels. Defeat enemies by sliding into them or stomping from above. Earn fireball charges per kill. Clear every token or defeat every enemy in the boss-room panel and an exit door appears. Your previous attempt replays as a translucent ghost, so every retry is a race against yourself.',
    playHeading: 'Play',
    playBody: 'The game runs in your browser. Use the keyboard (WASD / arrows + Space) or a gamepad. Press Escape to pause, R to restart a level.',
    playButton: 'Launch game',
    videoHeading: 'Gameplay footage',
    videoPlaceholder: 'Gameplay videos will appear here once the game is exported. Check back soon.',
    featuresHeading: 'Features',
    features: [
      'Fluid movement with coyote time and jump buffering',
      'Combat through momentum: slide, stomp, and fireball',
      'Seven levels: ink-drawn terrain built at runtime from SVG strokes',
      'Ghost runner replays your previous attempt on each level',
      'Moving platforms, crumbling platforms, springs, and spikes',
      'Three enemy types: snails, flyers, and a boss',
      'Ambient parallax void background with drifting ink motes',
      'Live speedrun clock, HUD with health and token progress',
      'End-of-run screen with total time and fastest-run tracking',
    ],
    techHeading: 'Built with',
    techBody: 'Godot 4.7, GDScript, custom SVG-to-terrain pipeline, and hand-drawn sprite art. All terrain geometry is parsed from ink-stroke data at runtime.',
    creditsHeading: 'Credits',
    credits: [
      'Background music from YouTube (Ambient / Cinematic track)',
      'Player and enemy sprites by ansimuz, via opengameart.org',
      'All other art, code, and level design by Ahmed Hussain',
    ],
    bugsHeading: 'Report a bug',
    bugsBody: 'Found something wrong? Send a description of what happened (and which level you were on) and I will fix it.',
    bugsName: 'Name (optional)',
    bugsEmail: 'Email (optional)',
    bugsDescription: 'What happened?',
    bugsDescriptionPlaceholder: 'Describe the bug and which level you were on\u2026',
    bugsSend: 'Send bug report',
    bugsSending: 'Sending\u2026',
    bugsSuccess: 'Thank you. Bug report sent - I will look into it.',
    bugsError: 'Something went wrong. Please try again or email me directly.',
  },

  room: {
    navLabel: 'Room navigation',
    monitorLabel: 'Enter the website',
    posterLabel: 'My Dress Up Darling Poster',
    saitamaLabel: 'Saitama Poster',
    bonsaiLabel: 'Windowsill bonsai tree',
    catanLabel: 'Longest road ever, click to play',
    booksLabel: 'Books on the shelf',
    vhsLabel: 'VHS tape, play it on the monitor',
    bookLabels: {
      'moby-dick': 'Moby-Dick, Herman Melville',
      'nineteen-eighty-four': 'Nineteen Eighty-Four, George Orwell',
      odyssey: 'The Odyssey, Homer',
    },
    reader: {
      close: 'Close the book',
      loading: 'Opening the book',
      failed: 'This book would not open. Try again in a moment.',
      prev: 'Previous page',
      next: 'Next page',
      contents: 'Contents',
      smaller: 'Smaller type',
      larger: 'Larger type',
      page: 'Page',
      of: 'of',
      publicDomain: 'public domain',
    },
    lampLabel: 'Desk lamp',
    coffeeLabel: 'Coffee mug',
    ipodLabel: 'iPod, skip the track',
    sideTableClockLabel: 'Digital clock toggle 24-hour time',
    sideTableDrawerLabel: 'Side table drawer, click to open or close',
    windowLabel: 'Window, click to change the time of day',
    posterClickHint: 'Nice poster!',
    clockTip: "It's {time}",
    hint: 'Click around to explore',
    skip: 'Skip to the website',
    metaTitle: "Ahmed's website",
    metaDescription: 'Step into my digital room, a cosy pixel-art space and the front door to my personal website.',
    audio: {
      play: 'Play',
      pause: 'Pause',
      skip: 'Skip',
      nowPlaying: 'Now playing',
      speakersLabel: 'Toggle speakers',
      volume: 'Volume',
    },
    discoveryTitle: 'Things found',
    discoveryFound: 'Found',
    discoveryLocked: 'Locked',
    discoveryLabels: {
      lamp: 'Desk lamp',
      drawer: 'Drawer',
      clock: 'Clock',
      music: 'Music',
      poster: 'My Dress Up Darling poster',
      saitama: 'Saitama poster',
      bonsai: 'Bonsai',
      catan: 'Catan',
      coffee: 'Coffee mug',
      ipod: 'iPod',
      paint: 'Paint',
      minesweeper: 'Minesweeper',
      snake: 'Snake',
      blackjack: 'Blackjack',
      solitaire: 'Solitaire',
      pong: 'Pong',
      breakout: 'Breakout',
      readme: 'README',
      legal: 'Legal',
      settings: 'Settings',
      terminal: 'Terminal',
      screensaver: 'Screensaver',
      night: 'Night sky',
      guestbook: 'Guestbook',
      books: 'Bookshelf',
      movie: 'Movie night',
    },

  },
  catan: {
    title: 'CATAN',
    back: 'Back to room',
    turn: 'Turn {turn}',
    rules: 'Rules',
    newGame: 'New game',
    close: 'Close',
    cancel: 'Cancel',
    players: 'Players',
    hand: 'Your hand',
    devCards: 'Development cards',
    eventLog: 'Game log',
    actions: 'Actions',
    vp: 'VP',
    cards: 'cards',
    card: 'card',
    dev: 'dev',
    knights: 'knights',
    road: 'road',
    turnMarker: 'Current turn',
    longestRoad: 'Longest Road',
    largestArmy: 'Largest Army',
    playable: 'playable',
    newCard: 'new',
    diceLabel: 'Dice',
    cancelHint: 'Escape or click the button again to cancel',
    skip: 'Skip to my turn',
    speed: {
      label: 'Bot speed',
      instant: 'Instant',
      fast: 'Fast',
      normal: 'Normal',
      slow: 'Slow',
    },
    settings: {
      title: 'Settings',
      sound: 'Sound',
      volume: 'Volume',
      animations: 'Animations',
      animationsHint: "Your device's reduce-motion setting also turns animations off",
      tooltips: 'Tooltips',
      tooltipsHint: 'Show hover tooltips across the game',
      on: 'On',
      off: 'Off',
    },
    tooltips: {
      skip: 'Skip to your turn',
      skipDisabled: 'Skip to your turn (no bots are waiting)',
      rules: 'Read the rules',
      newGame: 'Start a new game',
      roll: 'Roll the dice to produce resources. A 7 moves the robber',
      road: 'Build a road',
      settlement: 'Build a settlement',
      city: 'Upgrade a settlement to a city',
      buyDevCard: 'Buy a development card',
      trade: 'Trade resources with the bank or a bot',
      playCard: 'Play a development card',
      endTurn: 'End your turn',
    },
    status: {
      setupSettlement: '{player}: place a starting settlement',
      setupRoad: '{player}: place a starting road',
      preRoll: '{player}: roll the dice',
      main: '{player}: build, trade, or end turn',
      roadBuilding: '{player}: place free roads ({remaining} left)',
      moveRobber: '{player}: move the robber',
      steal: '{player}: choose who to steal from',
      discard: '{player}: discard {count} cards',
      gameOver: '{player} wins!',
      gameOverYou: 'You win!',
      botStalled: 'The bots stalled and the game cannot continue. Start a new game.',
    },
    board: {
      board: 'Catan board',
      vertex: 'the corner of {hexes}.',
      vertexCoast: 'the corner of {hexes} on the coast.',
      edge: 'the edge between {hexes}',
      edgeCoast: 'the coast edge next to {hexes}',
      hex: '{terrain} {number}',
      sea: 'Sea',
      zoomIn: 'Zoom in',
      zoomOut: 'Zoom out',
      fit: 'Fit board',
      fitShort: 'Fit',
      place: 'Place',
      build: 'Build',
      moveRobber: 'Move robber',
      cancel: 'Cancel',
      targets: {
        setupSettlement: 'Place starting settlement on {place}',
        setupRoad: 'Place starting road on {place}',
        settlement: 'Place settlement on {place}',
        city: 'Upgrade to city on {place}',
        road: 'Build road on {place}',
        robber: 'Move robber to {place}',
      },
      terrain: {
        lumber: 'Lumber',
        wool: 'Wool',
        grain: 'Grain',
        brick: 'Brick',
        ore: 'Ore',
        desert: 'Desert',
      },
    },
    resources: {
      brick: 'brick',
      lumber: 'lumber',
      wool: 'wool',
      grain: 'grain',
      ore: 'ore',
    },
    actionBar: {
      roll: 'Roll dice',
      playCard: 'Play card',
      road: 'Road',
      roadAria: 'Build road',
      settlement: 'Settlement',
      settlementAria: 'Build settlement',
      city: 'City',
      cityAria: 'Build city',
      buyDevCard: 'Buy card',
      buyDevCardAria: 'Buy development card',
      trade: 'Trade',
      endTurn: 'End turn',
      hint: 'Hint',
      hintHide: 'Hide hint',
      needResources: 'Need {resources}',
      noSpot: 'No legal spot',
      noPieces: 'No pieces left',
      deckEmpty: 'Deck is empty',
      rollFirst: 'Roll first',
    },
    cost: {
      road: 'Costs 1 brick and 1 lumber',
      settlement: 'Costs 1 brick, 1 lumber, 1 wool and 1 grain',
      city: 'Costs 2 grain and 3 ore',
      devCard: 'Costs 1 wool, 1 grain and 1 ore',
    },
    vertexInfo: {
      pips: '{pips} {pipWord}: {hexes}',
      pipSingular: 'pip',
      pipPlural: 'pips',
      harbourAny: '3:1 harbour',
      harbourResource: '2:1 {resource} harbour',
      robberSuffix: '(robber)',
    },
    diceViewer: {
      title: 'Dice',
      noRoll: 'No roll yet',
      robber: 'Robber!',
    },
    handBuilds: {
      brick: 'Builds roads and settlements',
      lumber: 'Builds roads and settlements',
      wool: 'Builds settlements and development cards',
      grain: 'Builds settlements, cities and development cards',
      ore: 'Builds cities and development cards',
    },
    devCardsPanel: {
      play: 'Play',
      newTag: 'New',
      newHint: 'Bought this turn, playable next turn',
      vpHidden: '+1 VP (hidden)',
      empty: 'No development cards',
    },
    playersTable: {
      vp: 'Victory points',
      resources: 'Resource cards in hand',
      dev: 'Development cards',
      knights: 'Knights played',
      road: 'Longest road length',
      overSeven: 'Over 7 cards: loses half on a 7',
      longestRoad: 'Longest Road',
      largestArmy: 'Largest Army',
    },
    dicePanel: {
      title: 'Dice',
      lastRolls: 'last {n} rolls',
      rolled: 'Rolled',
      expected: 'Expected',
      total: 'Total',
    },
    newGameDialog: {
      title: 'New game',
      you: 'You',
      name: 'Your name',
      namePlaceholder: 'You',
      color: 'Your colour',
      colors: { red: 'Red', blue: 'Blue', white: 'White', orange: 'Orange' },
      opponents: 'Opponents',
      players: 'Players',
      three: '3 players',
      four: '4 players',
      botLevel: 'Bot level',
      levels: { easy: 'Easy', normal: 'Normal', hard: 'Hard' },
      levelDescs: {
        easy: 'Easy: relaxed bots that trade generously',
        normal: 'Normal: solid bots',
        hard: 'Hard: sharp bots that block the leader',
      },
      game: 'Game',
      board: 'Board',
      boards: { balanced: 'Balanced', random: 'Random', starter: 'Starter' },
      boardDescs: {
        balanced: 'Balanced: fair spread of resources and numbers',
        random: 'Random: anything goes',
        starter: 'Starter: a fixed map for first games',
      },
      pointsToWin: 'Points to win',
      decrease: 'Decrease points to win',
      increase: 'Increase points to win',
      shorter: 'Shorter game',
      longer: 'Longer game',
      optionsTitle: 'Options',
      on: 'On',
      off: 'Off',
      friendlyRobber: 'Friendly robber',
      friendlyRobberDesc: 'The robber cannot target players with 2 points or fewer',
      botTrades: 'Bots trade',
      botTradesDesc: 'Bots may offer you trades on their turns',
      endsCurrent: 'Starting a new game ends the current one.',
      start: 'Start game',
      learn: 'Learn to play',
      cancel: 'Cancel',
    },
    tutorial: {
      step: 'Step {current} of {total}',
      next: 'Next',
      back: 'Back',
      exit: 'Exit',
      finish: 'Finish',
      notAllowed: 'The tutorial is waiting for a different action.',
      doAction: 'Make the highlighted move to continue',
      collapse: 'Collapse the lesson text',
      expand: 'Expand the lesson text',
      verb: { click: 'Click', tap: 'Tap' },
      verbLower: { click: 'click', tap: 'tap' },
      steps: {
        welcome: {
          title: 'Welcome to Catan',
          body:
            'You are red, and the first player to {target} victory points wins. The status banner at the top shows whose turn it is and what to do. Use Next for each lesson step, or Exit to leave the lesson.',
        },
        board: {
          title: 'The island',
          body:
            'Each hex produces one resource for nearby settlements when its number is rolled: lumber from green, wool from light green, grain from yellow, brick from red-brown, ore from grey. The dots under a number show how often it is rolled; 6 and 8 are most likely, so they are printed red. Zoom and pan the board with the + and - buttons, or drag it.',
        },
        'first-settlement': {
          title: 'Your first settlement',
          body:
            '{verb} the highlighted corner of Grain 8 and Ore 5 to place your first settlement; it adds 9 pips of production. Then {verbLower} the highlighted edge to place the road next to it, toward the middle of the island.',
        },
        'bots-and-second': {
          title: 'Bots place, then you',
          body:
            'Watch the bots place their starting settlements. When it is your turn again, place your second settlement on the highlighted corner of Brick 12 and Lumber 2, then its road. You collect one brick and one lumber for the hexes around that settlement.',
        },
        hand: {
          title: 'Your hand and costs',
          body:
            'Your hand shows the resource cards you hold. A road costs 1 brick and 1 lumber, a settlement costs 1 brick, 1 lumber, 1 wool and 1 grain, a city costs 2 grain and 3 ore, and a development card costs 1 wool, 1 grain and 1 ore. The build buttons show these costs whenever you build.',
        },
        roll: {
          title: 'Roll the dice',
          body:
            '{verb} Roll to roll the dice. Your first roll is an 8, and your settlement on Grain 8 pays you one grain. The log records every event.',
        },
        'build-road': {
          title: 'Build a road',
          body:
            'You have 1 brick and 1 lumber, exactly the cost of a road. {verb} the Road build button, then {verbLower} the highlighted edge. Roads connect your settlements and count toward Longest Road.',
        },
        'end-turn': {
          title: 'End your turn',
          body:
            'You have nothing else to do, so end your turn. The bots then take their turns; use Skip to jump back to yours, or change the bot speed in Settings. Hint (H) suggests a move, and Undo (U) takes back a build or bank trade.',
        },
        seven: {
          title: 'The robber',
          body:
            '{verb} Roll. Your next roll is a 7, and the tutorial gives you extra grain so you hold more than 7 cards. Discard 10 grain, then move the robber to the highlighted hex and steal one card from Bram.',
        },
        'trade-settlement': {
          title: 'Trade 4:1, then settle',
          body:
            'You have spare ore but no brick, and a settlement costs 1 brick. {verb} Trade and give 4 ore for 1 brick on the Bank tab. Then choose Settlement from the build buttons and place it on the highlighted corner.',
        },
        'buy-card': {
          title: 'Buy a development card',
          body:
            'You have 1 wool, 1 grain and 1 ore, the cost of a development card. {verb} Buy card; it will be a Knight. Cards bought this turn wait until your next turn, so end your turn.',
        },
        knight: {
          title: 'Play the Knight',
          body:
            'Before rolling, play your Knight card. Move the robber to the highlighted hex and steal a card. Play three Knights over the game to earn Largest Army, worth 2 victory points.',
        },
        winning: {
          title: 'Longest Road, cities and winning',
          body:
            'The first player with a road of 5 or more connected segments holds Longest Road, worth 2 victory points. Cities upgrade settlements and produce double. Keep playing until someone reaches {target} victory points on their own turn.',
        },
        free: {
          title: 'You are on your own now',
          body:
            'The lesson is over, so play this game freely from here. Bots may now offer you trades, and the Trade button opens offers from you to them. Hint (H) and Undo (U) stay available whenever you want help.',
        },
      },
    },
    trade: {
      title: 'Trade',
      tabBank: 'Bank',
      tabPlayers: 'Players',
      close: 'Close',
      bank: {
        give: 'Give',
        get: 'Get',
        rate: '{rate}:1',
        trade: 'Trade with bank',
        action: 'Trade {giveCount} {give} for 1 {get}',
      },
      players: {
        youGive: 'You give',
        youGet: 'You get',
        recipients: 'Offer to',
        cards: '{count} cards',
        card: '1 card',
        offersLeft: '{count} offers left',
        offersLeftOne: '1 offer left',
        makeOffer: 'Make offer',
        invalid: 'Both sides need at least one card, and no resource can be on both sides.',
        rollFirst: 'Roll first',
        notMain: 'Trading is only available in your main phase',
        noOffers: 'No more trade offers this turn',
        noCards: 'No cards to offer',
      },
      pending: {
        waiting: 'Waiting',
        declined: 'Declined',
        counterTerms: 'You give {give}, you get {get}',
        tradeWith: 'Trade with {name}',
        acceptCounter: 'Accept counter',
        withdraw: 'Withdraw offer',
      },
      incomingOffer: {
        title: 'Trade offer',
        offersYou: 'offers you',
        youGet: 'You get',
        youGive: 'You give',
        accept: 'Accept',
        decline: 'Decline',
        cannotAccept: 'You do not have {resources}',
      },
    },
    playCard: {
      title: 'Play a development card',
      empty: 'No playable development cards.',
      chooseOne: 'Choose one resource to take from every opponent.',
      chooseTwo: 'Choose two resources to take from the bank. You may pick the same one twice.',
      play: 'Play',
      close: 'Close',
      cards: {
        knight: { name: 'Knight', desc: 'Move the robber and steal one card from a player with a building on the robber\u2019s hex.' },
        roadBuilding: { name: 'Road Building', desc: 'Place two free roads.' },
        yearOfPlenty: { name: 'Year of Plenty', desc: 'Take any two resources from the bank.' },
        monopoly: { name: 'Monopoly', desc: 'Name a resource and take every copy of it from all opponents.' },
        victoryPoint: { name: 'Victory Point', desc: 'Worth 1 VP. Revealed at the end of the game.' },
      },
    },
    discard: {
      title: 'Discard cards',
      prompt: 'The robber struck! Select cards to discard.',
      header: 'Discard {count} cards',
      ready: 'Ready',
      yourCards: 'Your cards',
      discardPile: 'Discard pile',
      pileEmpty: 'Nothing in the pile yet',
      add: 'Add {resource} to the pile',
      remove: 'Remove {resource} from the pile',
      selected: '{count} of {total} selected',
      keep: 'You keep:',
      confirm: 'Discard',
    },
    steal: {
      title: 'Choose who to steal from',
      prompt: '{player}: steal one card from a player next to the robber.',
      choose: 'Steal from {name}',
      vp: '{vp} VP',
      cardsHeld: '{count} cards',
      cardHeld: '1 card',
    },
    gameOver: {
      title: 'Game over',
      winner: '{player} wins!',
      winnerYou: 'You win!',
      turns: 'in {turns} turns',
      firstTo: 'First to {points} points',
      scoresCaption: 'Final victory points',
      colPlayer: 'Player',
      colSettlements: 'Settlements',
      colCities: 'Cities',
      colVpCards: 'VP cards',
      colRoad: 'Longest Road',
      colArmy: 'Largest Army',
      colTotal: 'Total',
      colTurn: 'Turn',
      blockedCol: 'Blocked',
      tabsLabel: 'Results sections',
      tabDice: 'Dice',
      tabResources: 'Resources',
      tabPoints: 'Points over time',
      tabHighlights: 'Highlights',
      diceCaption: 'Dice totals',
      diceTotal: 'Total',
      diceRolled: 'Rolled',
      diceExpected: 'Expected',
      diceNoRolls: 'No dice were rolled.',
      diceOver: 'Most over: {total} ({actual} rolls, {expected} expected)',
      diceUnder: 'Most under: {total} ({actual} rolls, {expected} expected)',
      producedCaption: 'Resources produced by each player',
      blockedByRobber: 'blocked by the robber: {n}',
      vpCaption: 'Victory points over time',
      final: 'final',
      highlightMostRobbed: 'Most robbed: {player} ({count} cards)',
      highlightBestProducer: 'Best producer: {player} ({count} cards)',
      highlightTrades: 'Trades: {players} with players, {bank} with the bank',
      highlightLuckiest: 'Luckiest number: {total} (rolled {actual} times, {expected} expected)',
      highlightUnluckiest: 'Unluckiest number: {total} (rolled {actual} times, {expected} expected)',
      noHighlights: 'Nothing notable this game.',
      partial: 'These stats only cover the part of the game played since the update.',
    },
    rulesPanel: {
      title: 'Rules',
      sectionsLabel: 'Sections',
      goalTitle: 'Goal and scoring',
      goalBody: [
        'Reach the target number of victory points on your own turn to win. The target can be changed in the New game dialog.',
        'Settlements are worth 1 VP and cities 2 VP. Longest Road and Largest Army are worth 2 VP each. Victory Point development cards are worth 1 VP each and stay hidden until the end.',
      ],
      turnTitle: 'Your turn',
      turnBody: [
        'Roll the dice: every hex with that number produces for each settlement or city on it. A 7 produces nothing.',
        'Then build roads, settlements and cities, trade, and buy development cards in any order.',
        'You may play one development card on your turn. Cards bought this turn wait until your next turn. End your turn when you are done.',
      ],
      costsTitle: 'Building costs',
      costs: {
        road: 'Road',
        settlement: 'Settlement',
        city: 'City',
        devCard: 'Development card',
      },
      pieceLimit: 'Limit {n}',
      deckSize: 'Deck {n}',
      productionTitle: 'Production and the robber',
      productionBody: [
        'Each number token shows pips: more pips mean the number is rolled more often.',
        'Roll a 7 and every player with more than 7 cards discards half (rounded down). Then move the robber to any hex and steal one card from a player with a settlement or city there. The robber blocks that hex from producing.',
        'Friendly robber (a New game option): the robber cannot target players with 2 points or fewer while another hex is legal.',
      ],
      devCardsTitle: 'Development cards',
      devCardsBody: 'You may play one development card on your turn. Cards bought this turn wait until your next turn.',
      tradingTitle: 'Trading',
      tradingBody: [
        'Bank: trade 4 of one resource for 1 of another.',
        'Harbours: build on a 2:1 harbour to trade 2 of that resource for 1 of anything, or on a 3:1 harbour to trade 3 of one resource for 1 of anything.',
        'Players: on your turn you may offer trades to other players. They can accept, decline, or counter. You then pick which accepted or countered offer to confirm.',
      ],
      awardsTitle: 'Longest Road and Largest Army',
      awardsBody: [
        'The first player with a road of 5 or more segments holds Longest Road (2 VP).',
        'The first player with 3 knights played holds Largest Army (2 VP). Beat the holder\u2019s total to take the badge.',
      ],
      shortcutsTitle: 'Keyboard shortcuts',
      shortcuts: [
        { keys: 'R', action: 'Roll' },
        { keys: 'E', action: 'End turn' },
        { keys: '1', action: 'Road' },
        { keys: '2', action: 'Settlement' },
        { keys: '3', action: 'City' },
        { keys: '4', action: 'Buy a development card' },
        { keys: 'T', action: 'Trade' },
        { keys: 'P', action: 'Play a development card' },
        { keys: 'U', action: 'Undo' },
        { keys: 'H', action: 'Hint' },
        { keys: 'L', action: 'Log' },
        { keys: '+ and -', action: 'Zoom' },
        { keys: '0', action: 'Fit board' },
        { keys: 'Escape', action: 'Cancel' },
        { keys: '?', action: 'All shortcuts' },
      ],
    },
    log: {
      setupSettlement: '{player} placed a starting settlement',
      setupRoad: '{player} placed a starting road',
      setupResources: '{player} collected starting resources: {resources}',
      roll: '{player} rolled a {dice}',
      produce: '{player} collected {resources}',
      discard: '{player} discarded {resources}',
      robber: 'robber',
      robberMoved: '{player} moved the {robber} to {terrain}',
      stole: '{player} stole a card from {victim}',
      stoleResource: '{player} stole {resource} from {victim}',
      builtRoad: '{player} built a road',
      builtSettlement: '{player} built a settlement',
      builtCity: '{player} built a city',
      boughtDevCard: '{player} bought a development card',
      playedKnight: '{player} played a {card}',
      playedRoadBuilding: '{player} played {card}',
      playedYearOfPlenty: '{player} played {card}',
      playedMonopoly: '{player} played {card}',
      yearOfPlenty: '{player} took {resources} from the bank',
      monopoly: '{player} monopolised {resource}, taking {taken}',
      maritimeTrade: '{player} traded {giveCount} {give} to the bank for {get}',
      domesticTrade: '{player} traded with {partner}',
      tradeProposed: '{player} offered {give} for {get}',
      tradeAccepted: '{player} accepted the offer',
      tradeDeclined: '{player} declined the offer',
      tradeCountered: '{player} countered with {give} for {get}',
      tradeCancelled: '{player} withdrew the offer',
      longestRoad: '{player} took Longest Road',
      longestRoadName: 'Longest Road',
      longestRoadLost: 'Longest Road is up for grabs',
      largestArmy: '{player} took Largest Army',
      largestArmyName: 'Largest Army',
      turnEnded: '{possessive} turn ended',
      gameOver: '{player} won the game',
    },
    hover: {
      hex: '{terrain} {number}: rolls {odds}; produces {resource} for adjacent buildings',
      desert: 'Desert: produces nothing',
      robberPresent: 'Robber present',
      robber: 'The robber: this hex produces nothing until it moves',
      harborAny:
        '3:1 harbour: trade any 3 identical cards for 1 while you own a settlement or city on either corner',
      harborResource:
        '2:1 {resource} harbour: trade 2 {resource} for 1 of anything while you own a settlement or city on either corner',
      building: '{possessive} {kind}: {vp} VP',
      settlement: 'settlement',
      city: 'city',
      road: '{possessive} road: longest road {length}',
    },
    keyPanel: {
      title: 'Board key',
    },
    key: {
      robber: 'Robber: blocks the hex it stands on, moves on a 7 or a Knight, and steals a card',
      settlement: 'Settlement: 1 VP, collects one card from each adjacent hex',
      city: 'City: 2 VP, collects two cards from each adjacent hex',
      road: 'Road: connects your buildings; 5+ in a row can win Longest Road',
      numberToken: 'Number token: the rolled number produces for adjacent buildings; dots show how likely',
      harbours: 'Harbours: 3:1 trades any three identical cards, or 2:1 for the shown resource',
      terrain: {
        brick: 'Brick: builds roads and settlements',
        lumber: 'Lumber: builds roads and settlements',
        wool: 'Wool: builds settlements and development cards',
        grain: 'Grain: builds settlements, cities and development cards',
        ore: 'Ore: builds cities and development cards',
        desert: 'Desert: produces nothing',
      },
    },
    layout: {
      levels: {
        easy: 'Easy',
        normal: 'Normal',
        hard: 'Hard',
      },
      replyAccepts: 'Accepts',
      replyDeclines: 'Declines',
      replyCounter: 'Counters: ...',
      replyPending: '...',
      hiddenVp: '+{n} hidden',
      piecesLeft: '{n} left',
      piecesLeftTitle: 'Pieces left',
      piecesSettlement: 'Settlements left',
      piecesCity: 'Cities left',
      deckCount: 'Deck: {n}',
      handWarning: 'A 7 would make you discard half',
      bankTitle: 'Bank',
      openPlayer: 'Open player details',
      sheetTitles: {
        build: 'Build',
        cards: 'Cards',
        log: 'Log',
      },
      closeSheet: 'Close sheet',
      menu: 'Menu',
      key: 'Key',
      undo: 'Undo',
      undoDisabled: 'Nothing to undo',
      turnSeparator: 'Turn {turn} . {player}',
      newEvents: 'New events',
      buildModeStatus: 'Choose a spot on the board. Esc cancels',
      buildModeStatusTouch: 'Tap a spot on the board, then confirm',
      diceRolls: '{n} rolls',
      announceSummary: '{player} turn: {count} events',
      cardNotPlayable: 'Playable next turn',
      backToRoom: 'Back to room',
      rematch: 'Rematch',
      incomingOffer: {
        title: 'Trade offer',
        prompt: '{player} offers a trade:',
        accept: 'Accept',
        decline: 'Decline',
      },
      shortcuts: {
        title: 'Keyboard shortcuts',
        items: [
          { keys: 'R', label: 'Roll the dice' },
          { keys: 'E', label: 'End your turn' },
          { keys: '1', label: 'Build a road' },
          { keys: '2', label: 'Build a settlement' },
          { keys: '3', label: 'Build a city' },
          { keys: '4', label: 'Buy a development card' },
          { keys: 'T', label: 'Trade' },
          { keys: 'P', label: 'Play a card' },
          { keys: 'U', label: 'Undo' },
          { keys: 'H', label: 'Hint' },
          { keys: 'L', label: 'Log and bank' },
          { keys: '+ / -', label: 'Zoom the board' },
          { keys: '0', label: 'Fit the board' },
          { keys: 'Esc', label: 'Cancel or close' },
          { keys: '?', label: 'This list' },
        ],
      },
    },
    effects: {
      summary: '{player} turn: {summary}',
      summaryRolled: 'rolled {total}',
      summaryBuiltRoad: 'built a road',
      summaryBuiltRoads: 'built {count} roads',
      summaryBuiltSettlement: 'built a settlement',
      summaryBuiltSettlements: 'built {count} settlements',
      summaryBuiltCity: 'built a city',
      summaryBuiltCities: 'built {count} cities',
      summaryBoughtCard: 'bought a card',
      summaryBoughtCards: 'bought {count} cards',
      summaryPlayedCard: 'played {card}',
      summaryEvents: '{count} events',
      yourTurn: 'Your turn',
      playedCard: '{player} played {card}',
      awardTake: '{player} takes the {award} (+2 VP)',
      awardTakeYou: 'You take the {award} (+2 VP)',
      awardLost: '{award} is up for grabs',
      stoleResource: '{subject} stole 1 {resource} from {object}',
    },
  },

  desk: {
    back: 'Room',
    backAria: 'Back to the room',
    clickAgain: 'Click again to return to room',
    desktop: 'Desktop',
    clockLabel: 'Clock, {time}, switch to {n}-hour time',
    clockTitle: 'Switch to {n}-hour time',
    screenLabel: 'Desktop shortcuts',
    linkedin: 'LinkedIn',
    github: 'GitHub',
    readme: 'README',
    linkedinTip: 'My LinkedIn profile',
    githubTip: 'My GitHub profile',
    readmeTip: 'About this site',
    legal: 'Legal',
    legalTip: 'Privacy & terms',
    movie: 'Movie',
    movieTip: 'Put the tape in, watch it on the monitor',
    movieApp: {
      title: 'VHS player',
      play: 'Play',
      pause: 'Pause',
      fullscreen: 'Full screen',
      seek: 'Scrub through the film',
      mute: 'Mute',
      unmute: 'Unmute',
      tape: 'PRESS PLAY',
    },
    settings: 'Settings',
    settingsTip: 'Audio, clock, and effects',
    paint: 'Paint',
    music: 'Music',
    musicTip: 'Playlist, pick a track',
    minesweeper: 'Minesweeper',
    snake: 'Snake',
    paintTip: 'Doodle on a pixel canvas, it saves itself',
    minesweeperTip: 'Classic mines, room edition',
    snakeTip: 'Eat, grow, do not hit the walls',
    blackjack: 'Blackjack',
    blackjackTip: 'Beat the dealer to 21',
    solitaire: 'Solitaire',
    solitaireTip: 'Klondike, draw one or three',
    pong: 'Pong',
    pongTip: 'First to 7, against the CPU or a friend',
    breakout: 'Breakout',
    breakoutTip: 'Break every brick, keep the ball alive',
    arcade: {
      fullscreen: 'Full screen',
      exitFullscreen: 'Exit full screen',
      escHint: 'Esc to exit full screen',
      cards: {
        ranks: ['Ace', '2', '3', '4', '5', '6', '7', '8', '9', '10', 'Jack', 'Queen', 'King'],
        suits: { S: 'spades', H: 'hearts', D: 'diamonds', C: 'clubs' },
        card: '{rank} of {suit}',
        faceDown: 'Face-down card',
      },
    },
    // Each block below belongs to one game's component (its Labels interface).
    blackjackApp: {
      table: 'Blackjack table',
      bank: 'BANK {n}',
      best: 'BEST {n}',
      bet: 'Bet {n}',
      deal: 'Deal',
      clear: 'Clear',
      hit: 'Hit',
      stand: 'Stand',
      double: 'Double',
      split: 'Split',
      insurance: 'Insurance?',
      yes: 'Yes',
      no: 'No',
      rebet: 'Rebet',
      newBet: 'New bet',
      outOfChips: 'Out of chips',
      rebuy: 'Re-buy 1,000',
      shuffling: 'Shuffling...',
      blackjackPays: 'BLACKJACK PAYS 3 TO 2',
      sixDecks: '6 decks',
      soft: 'Soft {n}',
      chip: 'Add {n} to the bet',
      hand: 'Your hand: {cards}. {total}',
      handBlackjack: 'Your hand: {cards}. {total}, blackjack',
      handBust: 'Your hand: {cards}. Bust',
      dealerShows: 'Dealer shows {card}',
      dealerHand: 'Dealer hand: {cards}. {total}',
      resultBlackjack: 'Blackjack! +{n}',
      resultWin: 'You win +{n}',
      resultPush: 'Push',
      resultBust: 'Bust',
      resultLose: 'You lose',
      resultDealerBust: 'Dealer busts! +{n}',
      resultDealerBlackjack: 'Dealer blackjack',
      resultSplitWin: '{won} of {total} hands win, +{n}',
      resultSplitLoss: '{won} of {total} hands win, -{n}',
      resultSplitEven: '{won} of {total} hands win, even',
      tutorial: {
        button: 'How to play',
        title: 'How to play',
        close: 'Close',
        back: 'Back',
        next: 'Next',
        start: 'Start playing',
        page: '{n} / {total}',
        goal: {
          title: 'Goal',
          body: 'Get closer to 21 than the dealer without going over. Going over 21 is a bust, and a bust loses right away.',
          dealer: 'Dealer',
          you: 'You',
        },
        values: {
          title: 'Card values',
          body: 'Cards 2 through 10 count as their number. Jacks, queens and kings count as 10. An ace counts as 1 or 11, and a hand that can count an ace as 11 is soft.',
          blackjack: 'A + K = blackjack',
          soft: 'A + 6 = soft 17',
          bust: '10 + 5 + 9 = bust',
        },
        betting: {
          title: 'Betting',
          body: 'Click a chip to add it to your bet. The minimum bet is 5 and you can bet up to 500 before each deal.',
          clear: 'Clear the bet',
          deal: 'Deal the cards',
        },
        turn: {
          title: 'Your turn',
          body: 'Take cards one at a time or stand on what you have. Doubling and splitting are one-time moves.',
          hit: 'Take one more card',
          stand: 'Keep your hand and end your turn',
          double: 'Double the bet and take exactly one card',
          split: 'Split a pair into two hands',
        },
        dealer: {
          title: 'The dealer',
          body: 'The dealer has one card face down and must draw to 16, then stand on 17 or higher, including a soft 17.',
          insurance: 'When the dealer shows an ace you can take insurance. It costs half your bet and pays 2 to 1 if the dealer has blackjack.',
          yes: 'Take insurance',
          no: 'Decline insurance',
        },
        payouts: {
          title: 'Payouts',
          body: 'Winning pays 1 to 1 and blackjack pays 3 to 2. A push returns your bet.',
          rows: [
            { term: 'Win', pays: 'Pays 1 to 1' },
            { term: 'Blackjack', pays: 'Pays 3 to 2' },
            { term: 'Push', pays: 'Returns your bet' },
          ],
          saved: 'Your bankroll and best are saved automatically. If you run out of chips you can re-buy 1,000.',
        },
      },
    },
    solitaireApp: {
      table: 'Solitaire table',
      score: 'Score {n}',
      time: 'Time {t}',
      moves: 'Moves {n}',
      draw: 'Draw {n}',
      best: 'Best {n}',
      undo: 'Undo',
      auto: 'Auto',
      newGame: 'New',
      drawOne: 'Draw 1',
      drawThree: 'Draw 3',
      newGameMenu: 'New game options',
      winTitle: 'You win!',
      playAgain: 'Play again',
      winScore: 'Score {n}',
      winTime: 'Time {t}',
      winMoves: 'Moves {n}',
      winAria: 'You won Solitaire',
      illegal: "Can't move that there",
      stock: 'Stock',
      recycle: 'Recycle the waste',
      waste: 'Waste pile',
      foundation: 'Foundation {n}',
      tableau: 'Column {n}',
      cardAtTableau: '{card}, column {n}',
      cardAtWaste: '{card}, waste',
      cardAtFoundation: '{card}, foundation {n}',
    },
    pongApp: {
      court: 'Pong court',
      onePlayer: '1 Player',
      twoPlayers: '2 Players',
      difficulty: 'Difficulty',
      easy: 'Easy',
      normal: 'Normal',
      hard: 'Hard',
      hint: 'P1: W/S or mouse   P2: Up/Down   Space pauses',
      bestRally: 'Best rally {n}',
      paused: 'Paused',
      resume: 'Resume',
      menu: 'Menu',
      rematch: 'Rematch',
      finalScore: '{l} - {r}',
      youWin: 'YOU WIN',
      cpuWins: 'CPU WINS',
      player1Wins: 'PLAYER 1 WINS',
      player2Wins: 'PLAYER 2 WINS',
      scoreAnnounce: 'Score, {l} to {r}',
    },
    breakoutApp: {
      field: 'Breakout playfield',
      hudScore: 'SCORE',
      hudHi: 'HI',
      hudLevel: 'LEVEL',
      title: 'BREAKOUT',
      start: 'Click or press Space to launch',
      best: 'Best {n}',
      score: 'Score {n}',
      hint: 'Mouse or Left/Right  Space launches  P pauses',
      pause: 'Pause',
      paused: 'Paused',
      resume: 'Resume',
      levelClear: 'Level {n} clear! +{bonus}',
      lifeLost: 'Ball lost. Lives left: {n}',
      over: 'Game over',
      playAgain: 'Play again',
      announceOver: 'Game over. Score {n}. Best {best}',
    },
    paintApp: {
      pencil: 'Pencil',
      eraser: 'Eraser',
      fill: 'Fill',
      clear: 'Clear canvas',
      clearConfirm: 'Really clear?',
      download: 'Download PNG',
      color: 'Colour {n}',
      canvas: 'Pixel drawing canvas',
    },
    mines: {
      board: 'Minesweeper board',
      cell: 'Cell {r}, {c}',
      minesLeft: 'Mines: {n}',
      time: 'Time: {s}s',
      best: 'Best: {s}s',
      reset: 'New game',
      won: 'Cleared!',
      lost: 'Boom.',
    },
    snakeApp: {
      board: 'Snake board',
      score: 'Score: {n}',
      best: 'Best: {n}',
      reset: 'New game',
      over: 'Game over',
      won: 'Perfect run!',
      paused: 'Paused',
      resume: 'Resume',
      hint: 'Arrows or WASD  ·  Space pauses  ·  Enter restarts',
    },
    readmeApp: {
      title: 'README.txt',
      close: 'Close',
    },
    legalApp: {
      title: 'Legal.txt',
      privacyTab: 'Privacy',
      termsTab: 'Terms',
      close: 'Close',
    },
    settingsApp: {
      title: 'Settings',
      sfx: 'SFX',
      sfxVolume: 'SFX volume',
      musicVolume: 'Music volume',
      clock: '24-hour clock',
      clock12: '12 h',
      clock24: '24 h',
      on: 'ON',
      off: 'OFF',
      close: 'Close',
    },
    musicApp: {
      title: 'Room Jams',
      nowPlaying: 'Now playing',
      select: 'Play track',
    },
    guestbook: 'Guestbook',
    guestbookTip: 'Sign my guestbook',
    guestbookApp: { title: 'Guestbook', close: 'Close', namePh: 'your name', messagePh: 'leave a message', sign: 'Sign', empty: 'No messages yet. Be the first!', posting: 'Signing...', error: 'Could not post. Try again.' },
    terminalApp: {
      title: 'Terminal',
    },
  },

  notFound: {
    label: '404',
    heading: 'Page not found.',
    body: 'This page does not exist or has been moved.',
    home: 'Return home',
  },

  legal: {
    eyebrow: 'Legal',
    effectiveDate: 'Effective date',
    terms: {
      title: 'Terms of Use',
      date: '14 June 2026',
      s1h: '1. Acceptance',
      s1b: 'By accessing or using ahmedyhussain.com (the \u201cSite\u201d), you agree to be bound by these Terms of Use. If you do not agree, do not use the Site.',
      s2h: '2. Intellectual Property',
      s2b1: 'All content on this Site, including but not limited to text, design, layout, graphics, and code, is the intellectual property of Ahmed Hussain and is protected by Australian and international copyright law.',
      s2b2: 'Copyright \u00a9 Ahmed Hussain. All rights reserved.',
      s3h: '3. Prohibited Uses',
      s3intro: 'You expressly agree that you will not, without prior written permission:',
      s3items: [
        'Scrape, crawl, or otherwise automatically extract content from this Site',
        'Use any content from this Site to train, fine-tune, or otherwise develop artificial intelligence or machine learning models',
        'Ingest any content into vector databases, embedding stores, or similar retrieval systems for AI purposes',
        'Reproduce, redistribute, or republish content without attribution and permission',
        'Create derivative works based on content from this Site',
        'Use this Site in any manner that could interfere with its operation or impose an unreasonable load on its infrastructure',
      ],
      s4h: '4. AI and Automated Systems',
      s4p1: 'All content on this Site is copyrighted and may not be reproduced, redistributed, scraped, indexed for AI training, used in machine learning datasets, or incorporated into generative AI systems without prior written permission from Ahmed Hussain.',
      s4p2: 'Operators of AI crawlers and large language model training pipelines are on notice that access to this Site for the purpose of data collection is prohibited. This prohibition is reflected in the Site\u2019s robots.txt file and HTTP response headers.',
      s5h: '5. Accuracy of Information',
      s5b: 'Content on this Site is provided for informational purposes only and does not constitute legal advice. While I make reasonable efforts to ensure accuracy, I make no representations as to the completeness or timeliness of information provided.',
      s6h: '6. Limitation of Liability',
      s6b: 'To the extent permitted by law, Ahmed Hussain excludes all liability for loss or damage of any kind arising from your use of this Site or reliance on its content.',
      s7h: '7. Governing Law',
      s7b: 'These Terms are governed by the laws of the Australian Capital Territory, Australia. Any dispute arising from these Terms will be subject to the jurisdiction of the courts of the ACT.',
      s8h: '8. Contact',
      s8b: 'For permissions or queries regarding these Terms, contact: ahmedyhussain07@gmail.com',
    },
    privacy: {
      title: 'Privacy Policy',
      date: '11 July 2026',
      s1h: '1. Overview',
      s1b: 'This Privacy Policy explains how ahmedyhussain.com (\u201cSite\u201d) collects, uses, and stores personal information. I am committed to handling your data responsibly and in accordance with the Australian Privacy Act 1988 (Cth) and applicable Australian Privacy Principles.',
      s2h: '2. Information Collected',
      s2intro: 'When you sign the guestbook (on the in-room desktop), the Site stores your chosen name, your message, and a server-generated timestamp. The guestbook also rate-limits submissions by IP address; the IP is held transiently (one hour) and is not stored permanently.',
      s2items: [],
      s2after: 'Apart from the guestbook, this Site has no contact form and no user accounts. If you email me at the address on this Site, I receive whatever you choose to put in that email.',
      s3h: '3. How Information Is Used',
      s3intro: 'Guestbook entries are publicly visible on the in-room desktop. Any email you send me is used solely to:',
      s3items: ['Respond to your enquiry or message'],
      s3after: 'Your data is never sold or shared with third parties for marketing.',
      s4h: '4. Data Storage and Security',
      s4p1: 'Guestbook entries are stored in an Upstash Redis database. IP addresses used for rate-limiting expire automatically after one hour. Email you choose to send reaches me through my email provider and is transmitted using TLS encryption.',
      s4p2: 'I follow standard practices to keep any correspondence secure.',
      s5h: '5. Retention',
      s5b: 'Guestbook entries are kept indefinitely (the most recent 500 entries). You may request removal of your entry at any time by email. Email records are kept only as long as necessary to respond and for reasonable record-keeping.',
      s6h: '6. Cookies, Analytics and Local Storage',
      s6p1: 'This Site does not use tracking cookies, advertising pixels, or social media tracking scripts. A single cookie may be set only to remember your chosen language; it contains no personal information and is not used for tracking.',
      s6p2: 'The Site uses Vercel Speed Insights to measure anonymous, aggregated performance metrics such as page load times. It does not use cookies and does not identify you.',
      s6p3: 'The Site saves small preferences (such as your chosen language, audio and lamp state, and any drawing made in the Paint app) in your browser using local storage. That information stays on your device, is never transmitted to me or any third party, and you can clear it any time through your browser settings.',
      s7h: '7. Your Rights',
      s7b: 'Under the Australian Privacy Principles, you have the right to access, correct, or request deletion of personal information held about you. To exercise these rights, contact me at the address below.',
      s8h: '8. Contact',
      s8b: 'Privacy enquiries: ahmedyhussain07@gmail.com',
    },
  },

  tutoring: {
    eyebrow: 'Tutoring',
    headingLine1: 'Private tutoring,',
    headingLine2: 'Canberra.',
    intro1: 'I offer one-on-one tutoring for secondary school students in Canberra. My approach puts understanding ahead of memorisation. The aim is to build the kind of structured thinking that holds up in exams and well after them.',
    intro2: 'Sessions are available online and in person at the ANU campus or nearby locations.',
    seniorSecondary: 'Senior secondary',
    years1112: 'Years 11\u201312',
    middleSecondary: 'Middle secondary',
    years710: 'Years 7\u201310',
    allNonScience: 'All non-science subjects available',
    y1112: [
      { subject: 'Physics', note: 'ACT BSSS curriculum' },
      { subject: 'Mathematics', note: 'Methods & Specialist' },
      { subject: 'English', note: 'Literature & Language' },
      { subject: 'Legal Studies', note: 'ACT BSSS curriculum' },
    ],
    y710: ['Mathematics', 'English', 'History', 'Geography', 'Legal Studies', 'Commerce', 'Business', 'Economics'],
    pricing: 'Pricing',
    online: 'Online',
    onlinePrice: '$60',
    perHour: 'per hour',
    onlineFeatures: ['Via video call', 'Flexible scheduling', 'Screen sharing for worked examples'],
    inPerson: 'In Person',
    inPersonPrice: '$70',
    inPersonFeatures: ['ANU campus', 'Canberra by arrangement', 'Printed materials available'],
    faq: 'FAQ',
    faqs: [
      { q: 'Where do sessions take place?', a: 'Sessions are available online via video call, at the Australian National University campus, or at a Canberra location arranged in advance.' },
      { q: 'How long is each session?', a: 'Sessions are typically one hour. Longer sessions can be arranged on request.' },
      { q: 'What year levels do you tutor?', a: 'Years 7\u201310 for most non-science subjects, and Years 11\u201312 for Physics, Mathematics, English, and Legal Studies in line with the ACT BSSS curriculum.' },
      { q: 'Do you offer trial sessions?', a: 'Please get in touch to discuss your requirements. I am happy to discuss how I can best support your learning before committing.' },
      { q: 'How do I book a session?', a: 'Use the contact form below or email me directly. I will respond within one business day to arrange a suitable time.' },
    ],
    enquire: 'Enquire',
    enquireHeading: 'Book a session.',
    enquireBody: 'Use this form to enquire about tutoring. Include your year level and the subjects you need help with, and I will get back to you within one business day.',
  },
}

export type Dictionary = typeof en